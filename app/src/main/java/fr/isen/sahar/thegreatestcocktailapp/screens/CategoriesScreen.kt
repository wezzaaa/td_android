...
import fr.isen.sahar.thegreatestcocktailapp.ui.components.CategoryCard
        ...
        when {
            ...
            state.error != null -> ErrorView(state.error.orEmpty(), modifier)
                ...
                LazyColumn(
                    ...
                    item {
                        Text(
                            text = "Browse the list provided by TheCocktailDB and open a category to see all matching drinks.",
                            style = MaterialTheme.typography.bodyLarge,
                            color = MaterialTheme.colorScheme.onBackground.copy(alpha = 0.8f)
                        )
                    }
                    items(state.data.orEmpty()) { category ->
